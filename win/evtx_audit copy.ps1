...
function audit_files($audit_file, $host_name) {
    ...
    foreach ($event_code in $event_codes) {
        ...
        Start-Job -ScriptBlock {
            param($audit_file, $eventcode_1, $eventcode_2, $description)

            $result = if ($eventcode_2 -eq "") {
                Get-WinEvent -FilterHashtable @{ Path=$audit_file; Id=$eventcode_1 } -ErrorAction SilentlyContinue
            } else {
                Get-WinEvent -FilterHashtable @{ Path=$audit_file; Id=($eventcode_1, $eventcode_2) } -ErrorAction SilentlyContinue
            }

            if ($result) {
                $Global:all_win_events += @(
                    "################################################################################################################"
                    "#     EventCode: $eventcode_1       EventCode:$eventcode_2"
                    "#     Description: $description"
                    "################################################################################################################"
                    $result | Select-Object TimeCreated, LogName, Id, UserName, MachineName, Message, ProccessId, LevelDisplayName
                )
            }
        } -ArgumentList $audit_file, $eventcode_1, $eventcode_2, $description
        ...
        if ($jobCount -eq 10) {
            ...
            # Retrieve the results from all completed jobs
            $completedJobs = Get-Job | Receive-Job

            # Add the results to the global variable
            $Global:all_results += $completedJobs
        ...
        Start-Sleep -Milliseconds 100 # Introduce a delay between each iteration
    }

    # Wait for all remaining jobs to complete
    Get-Job | Wait-Job

    # Retrieve the results from all remaining jobs
    $remainingJobs = Get-Job | Receive-Job

    # Add the results to the global variable
    $Global:all_results += $remainingJobs
